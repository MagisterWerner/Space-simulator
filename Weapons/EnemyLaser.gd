...
func _on_EnemyLaser_body_entered(body: Node) -> void:
	if body.is_in_group("ASTEROIDS") or body.is_in_group("PLAYERS"):
		body.call_deferred("explode")
		_on_Lifetime_timeout()


# Trigger the explode-function on instanced Area2D-scenes and remove the instanced laser
func _on_EnemyLaser_area_entered(body: Area2D) -> void:
